...
permalink: /pbi-tools/release-notes.html
---

## [1.0.0-beta.7](https://github.com/action-bi-toolkit/pbi-tools/releases/tag/1.0.0-beta.7) - 2021-11-07

* [#16](https://github.com/action-bi-toolkit/pbi-tools/issues/16) **pbi-tools Core Version** released (with distributions for Win x64, Linux Desktop x64, Linux Alpine x64). This version is available cross-platform and supports CI/CD deployment and automation scenarios where a local Power BI Desktop installation is not available.
* [#16](https://github.com/action-bi-toolkit/pbi-tools/issues/16) Full/classic version of pbi-tools rebranded as "Desktop CLI" (exposed as "edition" `info` property)
* compile-pbix improvements: New `PbixWriter` API for Core version
* [#48](https://github.com/action-bi-toolkit/pbi-tools/issues/48) **New 'deploy' CLI Action**, initial release limited to Report-only PBIX deployments from a PbixProj folder using service principal
* [#46](https://github.com/action-bi-toolkit/pbi-tools/issues/46) `[compile-pbix]` Emit warning when creating PBIX output with model
...
* [#13](https://github.com/action-bi-toolkit/pbi-tools/issues/13) Hardened PBI Desktop API dependency: Improved forward-compatibility (IPowerBIPackage no longer statically implemented)
* [#22](https://github.com/action-bi-toolkit/pbi-tools/issues/22) `export-bim`: AAS datasources no longer generated by default (opt-in via "-generateDataSources")
* [#25](https://github.com/action-bi-toolkit/pbi-tools/issues/25) `info` Path of executing tool shown as _toolPath_
...
* `extract` Action
  * [#14](https://github.com/action-bi-toolkit/pbi-tools/issues/14) Support reading model from running Power BI Desktop instance. Specify port number via optional '-pbiPort' argument.
